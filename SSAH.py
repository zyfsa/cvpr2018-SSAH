...
class SSAH(object):
    ...
    def build_model(self):
        ...
        self.loss_i = alpha * Loss_pair_Fea_I + gamma * Loss_pair_Hsh_I + beta * Loss_quant_I + eta * Loss_label_I
        ...
        self.loss_t = alpha * Loss_pair_Fea_T + gamma * Loss_pair_Hsh_T + beta * Loss_quant_T + eta * Loss_label_T


    def train(self):
            # """Train"""
        
        self.load_zyf(self.checkpoint_dir, self.have_weight)
    ...
    def load_zyf(self, checkpoint_dir, have_weight):
        ...
        ckpt = tf.train.get_checkpoint_state(checkpoint_dir)
        if ckpt and ckpt.model_checkpoint_path and have_weight:
            ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
            print (ckpt_name)
            self.saver.restore(self.sess, os.path.join(checkpoint_dir, ckpt_name))
            print ("success finutune")
        return 
